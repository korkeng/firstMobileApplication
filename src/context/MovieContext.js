...
const historyReducer = (histtoryState, action) => {
    switch(action.type) {
        case 'SEARCH_MOVIE':
            const newState = histtoryState.find(data => data.name === action.payload)
            return newState ? histtoryState : [...histtoryState, { name: action.payload }]
        default:
...
const favouriteReducer = (favouriteState, action) => {
    switch(action.type) {
        case 'ADD_FAVOURITE':
            ...
            return newState;
        case 'DEL_FAVOURITE':
            const filteredState = favouriteState.filter(dat => dat.id !== action.payload)
...
export const MovieContextProvider = ({ children }) => {
    ...
    const addFavourite = (img, name, desc, vote, date, id) => {
        favouriteDispatch({type: 'ADD_FAVOURITE', payload: { img, name, desc, vote, date, id }})
    }
